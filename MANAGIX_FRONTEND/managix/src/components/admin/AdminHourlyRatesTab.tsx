...
const AdminHourlyRatesTab: React.FC = () => {
  ...
  const saveRate = async (row: any) => {
    ...
    if (Number.isNaN(rate) || rate < 0) {
      toast('Enter a valid hourly rate (0 or greater).');
      return;
    }
    setSavingId(userId);
    try {
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-2xl p-32 text-center">
        <div className="inline-block w-8 h-8 border-4 border-black border-t-transparent rounded-full animate-spin mb-4" />
        <p className="text-[#9CA3AF] font-extrabold uppercase tracking-[0.4em] text-[10px]">Loading payroll</p>
      </div>
    );
  }
    ...
    return (
      <div className="bg-white rounded-2xl border border-red-200 p-10">
        ...
        <button
          type="button"
          onClick={load}
          className="px-6 py-3 bg-black text-white rounded-xl text-[10px] font-extrabold uppercase"
        >
          Retry
        </button>
      </div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-2xl overflow-hidden">
      <div className="px-10 py-8 border-b border-[#F3F4F6]">
        <h2 className="text-xl font-extrabold uppercase tracking-tight">Compensation — hourly rates</h2>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-[#F3F4F6] bg-white">
              <th className="px-10 py-8 text-[10px] font-extrabold text-[#9CA3AF] uppercase tracking-[0.2em]">
                Personnel
              </th>
              <th className="px-10 py-8 text-[10px] font-extrabold text-[#9CA3AF] uppercase tracking-[0.2em]">
                Status
              ...
              <th className="px-10 py-8 text-[10px] font-extrabold text-[#9CA3AF] uppercase tracking-[0.2em] text-right">
                Execution
              </th>
            </tr>
          </thead>
            ...
            {rows.map((r) => {
              ...
              return (
                <tr key={uid} className="hover:bg-[#F9FAFB] transition-all group">
                  ...
                  <td className="px-10 py-6">
                    <span className="inline-block bg-[#F3F4F6] px-5 py-2.5 rounded-xl text-[10px] font-extrabold uppercase">
                      {roleLabel}
                    </span>
                  </td>
                  <td className="px-10 py-6">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-extrabold text-[#9CA3AF]">$</span>
                      <input
                        type="number"
                        min={0}
                        step={0.01}
                        className="w-28 bg-[#F3F4F6] border border-transparent hover:border-black focus:border-black px-4 py-2.5 rounded-xl text-sm font-extrabold outline-none transition-all"
                        value={rateVal}
                        placeholder="0.00"
                        onChange={(e) => updateRate(uid, e.target.value)}
                      />
                      <span className="text-[9px] font-extrabold text-[#9CA3AF] uppercase">/ hr</span>
                    </div>
                  </td>
                  <td className="px-10 py-6 text-right">
                    <button
                      ...
                      onClick={() => saveRate(r)}
                      className="bg-black text-white px-6 py-2.5 rounded-xl text-[9px] font-extrabold uppercase tracking-[0.2em] hover:bg-[#1A1A1A] transition-all active:scale-90 shadow-lg disabled:opacity-50"
                    >
        ...
        {rows.length === 0 && (
          <div className="p-32 text-center">
            <p className="text-[#9CA3AF] font-extrabold uppercase tracking-[0.5em] text-[10px]">No users found</p>
          </div>
        )}
      </div>
    </div>
