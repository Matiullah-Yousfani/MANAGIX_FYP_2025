  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-6">
      ...
      <motion.div
        ...
        className="relative bg-surface p-10 rounded-xl w-full max-w-lg shadow-e3 border border-line overflow-hidden"
      >
        <div className="relative z-10">
          <div className="mb-8">
            <label className="text-[10px] font-bold text-primary uppercase tracking-widest mb-2 block">Task Orchestration</label>
            <h2 className="text-2xl font-bold text-fg tracking-tight">Create New Task</h2>
            <p className="font-medium text-fg-muted mt-1">Assign responsibilities and link to milestones.</p>
          </div>

          <div className="space-y-5">
            {/* Title Input */}
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-fg-subtle uppercase tracking-widest ml-1 flex items-center gap-2">
                <FiType /> Task Title
              </label>
              <input
                disabled={isSubmitting}
                className="w-full p-4 bg-surface-2 text-fg border border-line rounded-lg outline-none focus:border-primary focus:ring-2 focus:ring-primary/25 font-bold transition-all disabled:opacity-50"
            ...
            <div className="space-y-1">
              ...
              <textarea
                disabled={isSubmitting}
                className="w-full p-4 bg-surface-2 text-fg border border-line rounded-lg outline-none focus:border-primary focus:ring-2 focus:ring-primary/25 font-medium leading-relaxed resize-none disabled:opacity-50"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                <select
                  disabled={isSubmitting}
                  className="w-full p-4 bg-surface-2 text-fg border border-line rounded-lg outline-none focus:border-primary focus:ring-2 focus:ring-primary/25 font-bold text-sm appearance-none cursor-pointer disabled:opacity-50"
            ...
            <div className="flex gap-4 pt-6">
              <button
                onClick={submit}
                disabled={isSubmitting}
                className="flex-[2] bg-primary text-primary-fg p-5 rounded-lg font-bold text-sm uppercase tracking-widest shadow-e2 hover:-translate-y-1 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
              >
              ...
              <button
                ...
                onClick={onClose}
                disabled={isSubmitting}
                className="flex-1 bg-surface-2 text-fg-muted p-5 rounded-lg font-bold text-sm uppercase tracking-widest transition-all hover:bg-surface-3 disabled:opacity-50"
              >
                Cancel
              </button>
            </div>
        ...
        <FiPlus className="absolute -bottom-6 -right-6 size-32 opacity-[0.03] text-fg rotate-12" />
      </motion.div>
