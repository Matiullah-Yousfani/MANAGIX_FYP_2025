...
const TaskHub = () => {
    ...
    return (
        <div className="p-8 max-w-5xl mx-auto min-h-screen">
            <h1 className="text-4xl font-extrabold mb-8 tracking-tighter italic">TASK EXPLORER</h1>
            
            <div className="grid md:grid-cols-2 gap-8">
                {/* Step 1: Project Selection */}
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200/70 flex flex-col h-[600px]">
                    <h3 className="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest mb-4 px-2">1. Select Project</h3>
                    <div className="relative mb-6 px-2">
                        <FiSearch className="absolute left-6 top-1/2 -translate-y-1/2 text-gray-400" />
                        <input 
                            type="text" 
                            placeholder="Search projects..." 
                            className="w-full pl-12 pr-4 py-3 bg-gray-50 border-none rounded-2xl text-sm outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
                            value={searchTerm}
                            ...
                            return (
                                <button 
                                    ...
                                    className={`w-full text-left p-4 rounded-2xl transition-all flex items-center justify-between group ${
                                        selectedProjectId === pId ? 'bg-indigo-600 text-white shadow-xl' : 'hover:bg-gray-50 text-gray-600'
                                    }`}
                                >
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200/70 flex flex-col h-[600px]">
                    <h3 className="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest mb-4 px-2">2. Select Milestone</h3>
                        ...
                        {!selectedProjectId ? (
                            ...
                            milestones.map(m => (
                                <button 
                                    ...
                                    className="w-full p-5 border-2 border-gray-50 rounded-[2rem] flex flex-col gap-2 hover:border-indigo-400 transition-all text-left group"
                                >
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs font-extrabold text-indigo-600 uppercase tracking-tighter">Milestone</span>
                                        <FiTarget className="text-gray-300 group-hover:text-indigo-600 transition-colors" />
                                    </div>
                                    <span className="font-extrabold text-gray-900 text-lg leading-tight">{m.title || m.Title}</span>
